...
class Area {
	 ...
	 */
	const SLUG = 'wop-wordpress-smtp-mail';
	...
	public function display_custom_auth_notices() {
		...
		switch ( $error ) {
			case 'google_access_denied':
				WP::add_admin_notice(
					...
					sprintf( esc_html__( 'There was an error while processing the authentication request: %s. Please try again.', 'wop-wordpress-smtp-mail' ), '<code>' . $error . '</code>' ),
			...
			case 'google_no_code_scope':
				WP::add_admin_notice(
					esc_html__( 'There was an error while processing the authentication request. Please try again.', 'wop-wordpress-smtp-mail' ),
					WP::ADMIN_NOTICE_ERROR
				);
				break;
			...
			case 'google_no_clients':
				WP::add_admin_notice(
					esc_html__( 'There was an error while processing the authentication request. Please make sure that you have Client ID and Client Secret both valid and saved.', 'wop-wordpress-smtp-mail' ),
					WP::ADMIN_NOTICE_ERROR
				);
				break;
		}
		...
		switch ( $success ) {
			case 'google_site_linked':
				WP::add_admin_notice(
					esc_html__( 'You have successfully linked the current site with you Google API project. Now you can start sending emails through Google.', 'wop-wordpress-smtp-mail' ),
	...
	public function add_admin_options_page() {
		...
		$this->hook = add_options_page(
			esc_html__( 'WOP Wordpress Mail SMTP Options', 'wop-wordpress-smtp-mail' ),
			esc_html__( 'WOP Wordpress Mail SMTP', 'wop-wordpress-smtp-mail' ),
			'manage_options',
			self::SLUG,
			array( $this, 'display' )
		);
	...
	public function enqueue_assets( $hook ) {
		...
		wp_enqueue_style(
			'wop-wordpress-smtp-mail-admin',
			wop_wordpress_mail_smtp()->plugin_url . '/assets/css/smtp-admin.min.css',
			false,
			WPMS_PLUGIN_VER
		);
		...
		wp_enqueue_script(
			'wop-wordpress-smtp-mail-admin',
			wop_wordpress_mail_smtp()->plugin_url . '/assets/js/smtp-admin' . WP::asset_min() . '.js',
			array( 'jquery' ),
			WPMS_PLUGIN_VER
		);
	}
	...
	public function display_admin_header() {
		...
		<div id="wop-wordpress-smtp-mail-header">
			<!--suppress HtmlUnknownTarget -->
			<img class="wop-wordpress-smtp-mail-header-logo" src="<?php echo wop_wordpress_mail_smtp()->plugin_url; ?>/assets/images/logo.png" alt="WOP Wordpress Mail SMTP"/>
	...
	public function get_admin_footer( $text ) {
		...
		if ( $this->is_admin_page() ) {
			$url = 'https://wordpress.org/support/plugin/wop-wordpress-smtp-mail/reviews/?filter=5#new-post';

			$text = sprintf(
				/* translators: %1$s - WP.org link; %2$s - same WP.org link. */
				__( 'Please rate <strong>WOP WordPress SMTP Mail </strong> <a href="%1$s" target="_blank" rel="noopener noreferrer">&#9733;&#9733;&#9733;&#9733;&#9733;</a> on <a href="%2$s" target="_blank">WordPress.org</a> . Thank you from the WOP WordPress SMTP team!', 'wop-wordpress-smtp-mail' ),
				$url,
				$url
			);
		}
	...
	public function display() {
		...
		<div class="wrap" id="wop-wordpress-smtp-mail">

			<div class="wop-wordpress-smtp-mail-page-title">
				<?php
			...
			<div class="wop-wordpress-smtp-mail-page wop-wordpress-smtp-mail-tab-<?php echo $this->get_current_tab(); ?>">
	...
	public function process_actions() {
		...
		if ( ! empty( $_POST ) ) {
			if ( ! empty( $_POST['wop-wordpress-smtp-mail'] ) ) {
				$post = $_POST['wop-wordpress-smtp-mail'];
			} else {
				$post = array();
			}
	...
	public function add_plugin_action_link( $links, $file ) {
		...
		if ( strpos( $file, 'wop-wordpress-smtp-mail' ) === false ) {
			return $links;
		}

		$settings_link = '<a href="' . $this->get_admin_page_url() . '">' . esc_html__( 'Settings', 'wop-wordpress-smtp-mail' ) . '</a>';
		...
		if ( ! empty( $wp_filter['user_admin_notices']->callbacks ) && is_array( $wp_filter['user_admin_notices']->callbacks ) ) {
			foreach ( $wp_filter['user_admin_notices']->callbacks as $priority => $hooks ) {
				foreach ( $hooks as $name => $arr ) {
					...
					if ( ! empty( $name ) && strpos( strtolower( $name ), 'wopmailsmtp' ) === false ) {
						unset( $wp_filter['user_admin_notices']->callbacks[ $priority ][ $name ] );
					}
				}
			}
		...
		if ( ! empty( $wp_filter['admin_notices']->callbacks ) && is_array( $wp_filter['admin_notices']->callbacks ) ) {
			foreach ( $wp_filter['admin_notices']->callbacks as $priority => $hooks ) {
				foreach ( $hooks as $name => $arr ) {
					...
					if ( ! empty( $arr['function'][0] ) && is_object( $arr['function'][0] ) && strpos( strtolower( get_class( $arr['function'][0] ) ), 'wopmailsmtp' ) !== false ) {
						continue;
					}
					if ( ! empty( $name ) && strpos( strtolower( $name ), 'wopmailsmtp' ) === false ) {
						unset( $wp_filter['admin_notices']->callbacks[ $priority ][ $name ] );
