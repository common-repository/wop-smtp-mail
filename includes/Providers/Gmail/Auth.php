...
use WOPMailSMTP\Options as PluginOptions;
use WOPMailSMTP\Providers\AuthAbstract;
...
class Auth extends AuthAbstract {
	...
	protected function include_google_lib() {
		require wop_wordpress_mail_smtp()->plugin_path . '/smtp/autoload.php';
	}
		...
		if ( ! $this->is_clients_saved() ) {
			wp_redirect(
				add_query_arg(
					'error',
					'google_no_clients',
					wop_wordpress_mail_smtp()->get_admin()->get_admin_page_url()
				)
			);
			exit;
		}
